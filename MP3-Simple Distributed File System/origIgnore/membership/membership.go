...
var introducer = "172.22.156.228"//vm 1
...
var localMembershipChannel chan map[int]*MemberNode
...
func copyMemberMap(theMap map[int]*MemberNode) map[int]*MemberNode {
	newMap := make(map[int]*MemberNode)
...
func pushMembershipUpdate() {
	localMembershipChannel <- copyMemberMap(memberMap)
}
...
func listenOtherPort() (err error) {
	...
	for {
		...
		//fmt.Println("Should be pushing membership update")
		pushMembershipUpdate()
	}
}
...
func MembershipRun(membershipChannel chan map[int]*MemberNode) {


	localMembershipChannel = membershipChannel

	logFile := "MembershipLog.log"
	...
	log.SetOutput(f)//just log to file
	//THIS IS HOW TO LOG AND PRINT AT SAME TIME
	...
	//wg.Wait()
	fmt.Println("Started membership handler")
	return
}
