...
// var serverIPs []string //TODO add IPs of all the machines. Doubt, how will you select randomly a IP for asking who is master?
...
func getMastersIP() string { //sync
	fmt.Println("getMastersIP() IP:" + serverIPs[0] + PORTForServer)
	conn, err := net.Dial("tcp", serverIPs[0]+PORTForServer) //make a connection to the one of the IPS. NEED to CHANGE this
	fmt.Print("getMastersIP() err:")
	fmt.Println(err)
	...
	fmt.Println(JSONStringForArguments)
...
func callServerForGet(replicaNodeIP string, JSONStringForArguments string) {
	...
	if operationSuccessStatus == "ok" {
		...
		if err != nil {
			fmt.Println("Could not write to file" + os.Args[3])
		}
	}
}
...
func main() {
	...
	// serverIPs = append(serverIPs, GetMyIP())
	fmt.Println(serverIPs[0])
	...
	case "get":
		...
		JSONStringForArgumentsForServer := encodeArgumentsToJSONString(os.Args[1]+"ForServer", os.Args[2])
		singleReplicaIDInStringFormat = masterIP //Remove this after debugging
